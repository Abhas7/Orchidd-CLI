...
// Token logic and constants are imported from ../../../lib/token.js at line 20.
...
export async function loginAction(opts) {
    ...
    // TODO : CHANGE THIS WITH TOKEN MANAGEMENT UTLIS
    ...
    try {
        ...
        console.log(`Please visit ${chalk.underline.blue(verification_url ||
            verification_url_complete)}`)
        ...
        if (!isCancel(shouldOpen) && shouldOpen) {
            const urlToOpen = verification_url_complete || verification_url;
            if (urlToOpen) {
                await open(urlToOpen);
            } else {
                console.log(chalk.yellow("Warning: No verification URL available to open."));
            }
        }

        console.log(
            chalk.gray(
                `Waiting for the authorization (expires in ${Math.floor(
                    expires_in / 60
                )} minutes)...`
            )
        );
